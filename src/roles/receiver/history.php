...
function uploadImage(string $field, string $directory): ?string
{
    if (empty($_FILES[$field]['name']) || ($_FILES[$field]['error'] ?? UPLOAD_ERR_NO_FILE) === UPLOAD_ERR_NO_FILE)
        return null;
    if (($_FILES[$field]['error'] ?? UPLOAD_ERR_OK) !== UPLOAD_ERR_OK || ($_FILES[$field]['size'] ?? 0) > 5 * 1024 * 1024)
        throw new RuntimeException('Upload an image no larger than 5 MB.');
    ...
    if (!$imageInfo || !isset($extensions[$imageInfo[2]]))
    ...
    if (!move_uploaded_file($_FILES[$field]['tmp_name'], $directory . DIRECTORY_SEPARATOR . $fileName))
...
mysqli_stmt_bind_param($userStatement, 'i', $receiverId);
mysqli_stmt_execute($userStatement);
$receiver = mysqli_fetch_assoc(mysqli_stmt_get_result($userStatement));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        if (!hash_equals($_SESSION['history_csrf'], (string) ($_POST['csrf_token'] ?? '')))
            throw new RuntimeException('Your session has expired. Please try again.');
        $bookingId = filter_input(INPUT_POST, 'booking_id', FILTER_VALIDATE_INT);
        $action = $_POST['action'] ?? '';
        if (!$bookingId || !in_array($action, ['review', 'report'], true))
            throw new RuntimeException('Invalid history item.');
        ...
        mysqli_stmt_bind_param($ownershipStatement, 'ii', $bookingId, $receiverId);
        mysqli_stmt_execute($ownershipStatement);
        $ownedBooking = mysqli_fetch_assoc(mysqli_stmt_get_result($ownershipStatement));
        mysqli_stmt_close($ownershipStatement);
        if (!$ownedBooking)
            throw new RuntimeException('You can only act on your own collected bookings.');

        if ($action === 'review') {
            $rating = filter_input(INPUT_POST, 'rating', FILTER_VALIDATE_INT);
            $comment = trim((string) ($_POST['comment'] ?? ''));
            if (!$rating || $rating < 1 || $rating > 5 || $comment === '')
                throw new RuntimeException('Choose a rating and enter a review comment.');
            ...
            if (!mysqli_stmt_execute($reviewStatement))
            ...
            if ($issueType === '' || $comment === '')
                throw new RuntimeException('Select an issue type and describe the problem.');
            ...
            if (!mysqli_stmt_execute($reportStatement))
                throw new RuntimeException('Unable to submit the report.');
            mysqli_stmt_close($reportStatement);
            $message = 'Your report has been submitted for review.';
        }
        $_SESSION['history_notice'] = $message;
...
if (isset($_SESSION['history_notice'])) {
    $message = (string) $_SESSION['history_notice'];
    $messageType = 'success';
...
<head>
    ...
    <title>History - Receiver - FoodBridge</title>
...
<body>
    ...
    <div class="dashboard-wrapper">
        <main class="dashboard-content">
            <h1 class="page-heading">Collection History</h1>
            <p class="page-subheading">Review completed food collections and report any issue to the admin team.</p>
            <?php if ($message): ?>
                <p class="history-notice <?php echo h($messageType); ?>" role="status"><?php echo h($message); ?></p>
            <?php endif; ?>
            <section class="history-panel" aria-label="Collection history">
                <div class="history-list"><?php if (!$history): ?>
                        <p class="empty-history">You have no completed collections yet.</p>
                    <?php else:
