...
$stmt = $pdo->query("
    ...
    WHERE role = 'donor'
");
$foodRescued = $stmt->fetch(PDO::FETCH_ASSOC)['total_rescued'];
...
$stmt = $pdo->query("
    ...
    WHERE status = 'active'
");
$conn = mysqli_connect("localhost", $username, $password, "foodbridge");
$activeDonations = $stmt->fetch(PDO::FETCH_ASSOC)['active_count'];
...
");
$readyToday = $stmt->fetch(PDO::FETCH_ASSOC)['ready_today'];
...
$stmt = $pdo->query("
    ...
    WHERE status = 'warned'
");
$flaggedUsers = $stmt->fetch(PDO::FETCH_ASSOC)['flagged_count'];
...
// 5. Impact snapshot
// ------------------------------------------------------------
$stmt = $pdo->query("SELECT COUNT(*) AS open_reports FROM reports WHERE status = 'active'");
$openReports = $stmt->fetch(PDO::FETCH_ASSOC)['open_reports'];
...
$stmt = $pdo->query("
    ...
    WHERE status = 'active'
");
$activePartners = $stmt->fetch(PDO::FETCH_ASSOC)['active_partners'];
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
  ...
  <header class="dashboard-header">
    <a href="admin-overview.php" class="navbar-brand">
    ...
    <div class="nav-overlay" id="navOverlay">
      <nav class="dashboard-nav">
        <a href="admin-overview.php" class="dashboard-nav-item active">Overview</a>
    ...
    <main class="dashboard-content">
      ...
      <div class="content-body admin-dashboard">
        <div class="summary-grid">
          <article class="summary-card">
            <div class="card-header-with-icon">
              ...
              <div class="icon-circle bg-lime-light">
                <svg viewBox="0 0 24 24" width="20" height="20" stroke="var(--color-forest)" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round">
                  <path d="M7 7h10"></path>
          ...
          <article class="summary-card">
            <div class="card-header-with-icon">
              ...
              <div class="icon-circle bg-terracotta-light">
                <svg viewBox="0 0 24 24" width="20" height="20" stroke="var(--color-terracotta)" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round">
          ...
          <article class="summary-card">
            <div class="card-header-with-icon">
              <span class="summary-label">Flagged Users</span>
              <div class="icon-circle bg-forest-light">
                <svg viewBox="0 0 24 24" width="20" height="20" stroke="var(--color-forest)" stroke-width="2.5" fill="none" stroke-linecap="round" stroke-linejoin="round">
                  <path d="M12 9v4"></path>
            ...
            <small>Accounts needing review</small>
          </article>
        </div>
              ...
              <?php else: ?>
                <?php foreach ($queue as $item): ?>
                  ...
                  <article class="pickup-card" tabindex="0" role="button">
                    ...
                    <a class="btn btn-sm <?= $isSoon ? 'btn-primary' : 'btn-accent' ?>" href="donations.php?id=<?= (int)$item['donation_id'] ?>">
                      View Details
                    </a>
                  </article>
                <?php endforeach; ?>
          ...
          <aside class="dashboard-side">
            <section class="panel-card impact-panel">
              ...
              <ul class="tag-list">
                ...
                <li><?= htmlspecialchars($activePartners) ?> active partner<?= $activePartners == 1 ? '' : 's' ?></li>
